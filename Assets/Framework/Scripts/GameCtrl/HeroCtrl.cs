...
namespace HolyTech.Ctrl
{
    public class HeroCtrl : Singleton<HeroCtrl>
    {
        public void Enter()
        {
            ...
            EventCenter.Broadcast(GameEventEnum.GameEvent_HeroEnter);
        }

        public void Exit()
        {
            EventCenter.Broadcast(GameEventEnum.GameEvent_HeroExit);
		...
		public void AddPreSelectHero(uint index,int heroId)
        {
            ...
            EventCenter.Broadcast(GameEventEnum.GameEvent_HeroPreSelect);
        }
        ...
        {      
			foreach(var item in PlayerManager.Instance.AccountDic.Values){
                ...
                if (index == PlayerManager.Instance.LocalAccount.GameUserSeat)
                {
                    ...
                    {
                        EventCenter.Broadcast<int>(GameEventEnum.GameEvent_HeroRealSelect, heroId);
                    }
                    else
                    {
                       //根据选择的英雄播放对应英雄口号
            ...
            EventCenter.Broadcast(GameEventEnum.GameEvent_HeroAddSelect);
		}

        public void ReconnectPreSelect(int heroId)
        {
            EventCenter.Broadcast<int>(GameEventEnum.GameEvent_HeroReconnectPreSelect,heroId);
        }
       ...
       private void Init()
		...
		private void EnterRandomHero()
        {
            ...
            EventCenter.Broadcast(GameEventEnum.GameEvent_HeroEnterRandom);
		}
