...
public class UIAltarSelect : MonoBehaviour {
    ...
    void OnEnable()
    {
        ...
        EventCenter.AddListener<Iplayer>(GameEventEnum.GameEvent_HeroDeathTime, ReMoveCurrGameObj);//死亡后重置祭坛
        EventCenter.AddListener<Iplayer>(GameEventEnum.GameEvent_HeroBackTown, ReMoveCurrGameObj);//回城后重置祭坛
        mStartEffect = false;
        mSetupEffect = false;
        mEffectTime = 0f;
    }
