...
public class VirtualStickUI : MonoBehaviour 
{
   ...
   public void SetVirtualStickUsable(bool enable)
	...
	{		
        ...
        HolyTech.Ctrl.UIGuideCtrl.Instance.AddUiGuideEventBtn(btnSelf.gameObject);//将要触发事件的新手引导的按钮放入列表
   ...
   void PressVirtual(int ie,bool isDown)
	...
	void SendMove()
	{
        ...
        //SendTime = Time.time; 
        CGLCtrl_GameLogic.Instance.EmsgToss_AskMoveDir(dir);//向服务器请求移动
        beforeDir = dir;
    ...
    private void PlayerAdMove(Vector3 mvDir) {
        ...
        if (HolyTech.Skill.BuffManager.Instance.isHaveStopBuff(player.GameObjGUID)) return;    
