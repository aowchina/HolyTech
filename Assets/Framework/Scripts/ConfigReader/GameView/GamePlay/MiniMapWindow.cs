...
namespace HolyTech.View
{
    public class MiniMapWindow : BaseWindow
    {
        ...
        { 
            EventCenter.AddListener(GameEventEnum.GameEvent_UpdateMiniMap, OnUpdateMapEvent);//更新小地图   消息管理器Update()广播 
            EventCenter.AddListener(GameEventEnum.GameEvent_InitMiniMap, InitGameMap);//初始化地图 当服务器返回消息要加载游戏对象时广播
            EventCenter.AddListener<Ientity>(GameEventEnum.GameEvent_AddMiniMap, OnAddMapEvent);//添加地图元素   当服务器返回消息要加载游戏对象时广播
            EventCenter.AddListener<UInt64>(GameEventEnum.GameEvent_RemoveMiniMap, OnRemoveMapEvent);//移除地图元素   当服务器返回 游戏对象消失的消息时广播
            EventCenter.AddListener<UInt64 , uint , UInt64>(GameEventEnum.GameEvent_BroadcastBeAtk, OnWarningEvent);//
            EventCenter.AddListener<UInt64>(GameEventEnum.GameEvent_RemoveMiniWarning, RemoveMapWarning);//移除地图警告
        ...
        {
            EventCenter.RemoveListener(GameEventEnum.GameEvent_UpdateMiniMap, OnUpdateMapEvent);
            EventCenter.RemoveListener(GameEventEnum.GameEvent_InitMiniMap, InitGameMap);
            EventCenter.RemoveListener<Ientity>(GameEventEnum.GameEvent_AddMiniMap, OnAddMapEvent);
            EventCenter.RemoveListener<UInt64>(GameEventEnum.GameEvent_RemoveMiniMap, OnRemoveMapEvent);
            EventCenter.RemoveListener<UInt64, uint, UInt64>(GameEventEnum.GameEvent_BroadcastBeAtk, OnWarningEvent);
            EventCenter.RemoveListener<UInt64>(GameEventEnum.GameEvent_RemoveMiniWarning, RemoveMapWarning);
        }
        ...
        private void AddMapElement(UInt64 guid, EntityCampType type, float x, float y, float z)
        {
            ...
            if (index <= (int)EntityCampType.CampTypeNull) return; //判断阵营是否是空
            ...
            if (element == null)
            {
                ...
                EventCenter.Broadcast(GameEventEnum.GameEvent_ChangeMapState);
            }
        ...
        private void DestroyMapElement(UInt64 guid)
        {
            ...
            GameObjectPool.Instance.ReleaseGO(element.resPath, element.gameObject, PoolObjectTypeEnum.POT_MiniMap);
        ...
        private void Clear()
        {
            for (int i = mMapElementDic.Count - 1; i >= 0; i--)
            {
                ...
                GameObjectPool.Instance.ReleaseGO(mMapElementDic.ElementAt(i).Value.resPath, mMapElementDic.ElementAt(i).Value.gameObject, PoolObjectTypeEnum.POT_MiniMap);
            }
            mMapElementDic.Clear();
        }
        ...
        private string GetNpcElementPath(Ientity entity,bool blueTeam)
        {
            ...
            {
                if (entity.NPCCateChild == ENPCCateChild.eNPCChild_SmallMonster)//小怪
                {
                    path = GameDefine.GameConstDefine.NpcMinimapCreepsIconSmall;//模糊的骷髅   
                }
                else if (entity.NPCCateChild == ENPCCateChild.eNPCChild_HugeMonster)// 大怪
                {
                    path = GameDefine.GameConstDefine.NpcMinimapCreepsIconHuge;//清楚的骷髅  
                }
                else if (entity.NPCCateChild == ENPCCateChild.eNPCChild_BUILD_Altar)//小兵基地
                {
                    path = GameDefine.GameConstDefine.NpcMinimapBarracksIcon;//盾牌 
                }
                else if (entity.NPCCateChild == ENPCCateChild.eNPCChild_BUILD_Base)//水晶塔 基地
                {
                    path = GameDefine.GameConstDefine.NpcMinimapBaseIcon;//清晰的牛角   
                }
                else if (entity.NPCCateChild == ENPCCateChild.eNPCChild_BUILD_Shop)//商店图标
                {
                    path = GameDefine.GameConstDefine.NpcMinimapShopIcon; //金钱   
                }
                else if (entity.NPCCateChild == ENPCCateChild.eNPCChild_BUILD_Tower)//攻击箭塔
                { 
