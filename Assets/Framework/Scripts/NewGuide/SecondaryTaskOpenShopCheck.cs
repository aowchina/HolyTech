...
namespace HolyTech.GuideDate
{
    public class SecondaryTaskOpenShopCheck : SecondaryTaskCheckBase
    {
        ...
        {            
            EventCenter.AddListener<CEvent>(GameEventEnum.GameEvent_NotifyOpenShop, OnEvent);             
        }

        public override void RemoveAddListener()
        {
            if (EventCenter.mEventTable.ContainsKey(GameEventEnum.GameEvent_NotifyOpenShop))
            {
                EventCenter.RemoveListener<CEvent>(GameEventEnum.GameEvent_NotifyOpenShop, OnEvent);
            }
        }
