...
namespace HolyTech.GuideDate
{
    public class SecondaryTaskInfo
    {
        ...
        public void OnEnter()
        { 
            EventCenter.AddListener<CEvent>((GameEventEnum)data.helpTriggerEvent, OnEventStart); 
        }
        ...
        {           
            ...
            if (data == null)
                return;
            if (EventCenter.mEventTable != null && EventCenter.mEventTable.ContainsKey((GameEventEnum)data.helpTriggerEvent))
            {
                EventCenter.RemoveListener<CEvent>((GameEventEnum)data.helpTriggerEvent, OnEventStart);
            }
            ...
            EventCenter.AddListener<CEvent>((GameEventEnum)(data.helpTriggerEvent + endTaskBetween), OnEventEnd); 
        }

        void OnEventEnd(CEvent eve) {
            if (EventCenter.mEventTable.ContainsKey((GameEventEnum)(data.helpTriggerEvent + endTaskBetween)))
            {
                EventCenter.RemoveListener<CEvent>((GameEventEnum)(data.helpTriggerEvent + endTaskBetween), OnEventEnd); 
            }   
            if (taskEffect != null)
            {
                taskEffect.OnEnd();
