...
namespace HolyTech.View
{
    public class SkillWindow : BaseWindow
    {
        ...
        {
            EventCenter.AddListener<EFuryState>(GameEventEnum.GameEvent_FuryStateInfo, ChangeFuryState);
            EventCenter.AddListener<int, string, bool>(GameEventEnum.GameEvent_AbsorbResult, OnAbsorbResult);
            EventCenter.AddListener<SkillTypeEnum, float, float>(GameEventEnum.GameEvent_LocalPlayerSkillCD, OnSkillCountDown);
            EventCenter.AddListener(GameEventEnum.GameEvent_LocalPlayerInit, OnLocalPlayerInit);
            EventCenter.AddListener<bool>(GameEventEnum.GameEvent_LocalPlayerSilence, OnLocalPlayerSilence);
        }

        //游戏事件注消
        protected override void OnRemoveListener()
        {
            EventCenter.RemoveListener<EFuryState>(GameEventEnum.GameEvent_FuryStateInfo, ChangeFuryState);
            EventCenter.RemoveListener<int, string, bool>(GameEventEnum.GameEvent_AbsorbResult, OnAbsorbResult);
            EventCenter.RemoveListener<SkillTypeEnum, float, float>(GameEventEnum.GameEvent_LocalPlayerSkillCD, OnSkillCountDown);
            EventCenter.RemoveListener(GameEventEnum.GameEvent_LocalPlayerInit, OnLocalPlayerInit);
            EventCenter.RemoveListener<bool>(GameEventEnum.GameEvent_LocalPlayerSilence, OnLocalPlayerSilence);
        }
        ...
        {
            EventCenter.Broadcast<bool, SkillTypeEnum, Iplayer>(GameEventEnum.GameEvent_SkillDescribleType, false, SkillTypeEnum.SKILL_NULL, PlayerManager.Instance.LocalPlayer);
            mIsShowDes = false;
            mCurSkillPress = -1;
            mTimePressStart = 0f;
        }
        ...
        public override void Update(float deltaTime)
        {
            ...
            {
                EventCenter.Broadcast<bool, SkillTypeEnum, Iplayer>(GameEventEnum.GameEvent_SkillDescribleType, true, GetSkillType(mCurSkillPress), PlayerManager.Instance.LocalPlayer);
        ...
        private void OnSkillPress(int ie, bool isDown)
        {
            ...
            //EventCenter.Broadcast<bool, SkillType, Iplayer>(GameEventEnum.GameEvent_SkillDescribleType, false, SkillType.SKILL_NULL, PlayerManager.Instance.LocalPlayer);
        ...
        private void OnSkillBtnFunc(int ie, bool isDown)//按技能
        {
            ...
            if (PlayerManager.Instance.LocalPlayer.FSM == null ||  //状态机是否为空
                PlayerManager.Instance.LocalPlayer.FSM.State == HolyTech.FSM.FsmState.FSM_STATE_DEAD || //状态是否为死亡状态
                HolyTech.Skill.BuffManager.Instance.isSelfHaveBuffType(1017))  //此时是否属于沉默  
            {
                return;
            }
            //向服务器发送消息，准备释放技能
