...
namespace HolyTech.GameEntity
{
    public class Iselfplayer : Iplayer
    {
        ...
        public void OnFocusTargetChanged()
        {
            ...
            else if (SyncLockTarget.EntityCamp == EntityCampType.CampTypeBad)//全敌对  黄色的
            {
                setCircleEnable(CircleGreen, false);
        ...
        private void setCircleEnable(GameObject obj, bool bEnabled)
        {
            ...
            if (bEnabled == true)
            {
                if (SyncLockTarget != null && SyncLockTarget.realObject != null)
                {
                    ...
                    float radius = 1.0f;
                    if (CTools.IfTypeHero((Common.GameData.EObjectType)SyncLockTarget.NpcGUIDType))
                    {
        ...
        {
            base.OnEnterFree();
            HolyTech.AudioManager.Instance.StopHeroAudio();//停止音效
        }
        ...
        {
            HolyTech.AudioManager.Instance.PlayHeroAudio(WalkAudioClip);//播放行走音效
            base.OnEnterMove();
        }
        ...
        public override void OnEnterDead()
        {
            ...
            HolyTech.AudioManager.Instance.PlayEffectAudio(DeadAudioClip);//播放死亡音效
